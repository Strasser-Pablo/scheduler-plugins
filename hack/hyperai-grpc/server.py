...
try:
    ...
    logging.warning("Kubernetes client not available - using simple hostname resolution")
...
class HyperAIServicer(hyperai_pb2_grpc.HyperAIServicer):
    def __init__(self):
        ...
        if KUBERNETES_AVAILABLE:
            try:
                ...
                logger.info("Loaded in-cluster Kubernetes configuration for node discovery")
            except Exception:
                try:
                    # Fallback to local kubeconfig
                    config.load_kube_config()
                    self.k8s_client = client.CoreV1Api()
                    logger.info("Loaded local Kubernetes configuration for node discovery")
    ...
    def GetScore(self, request, context):
        ...
        logger.info(f"  pod_json: {request.pod_json[:200]}{'...' if len(request.pod_json) > 200 else ''}")
        logger.info(f"  node_json: {request.node_json[:200]}{'...' if len(request.node_json) > 200 else ''}")
        ...
        try:
            ...
            node_score = self._call_node_agent(node_name, request.pod_json, request.node_json, pod_name, pod_namespace)
            ...
            if node_score is not None:
                ...
                final_score = 88
                logger.info(f"⚠️  Using fallback score (node agent unavailable): {final_score}")
    ...
    def _call_node_agent(self, node_name, pod_json, node_json, pod_name, pod_namespace):
        ...
        try:
            ...
            logger.info(f"Attempting to connect to node agent at: {node_agent_address} (node: {node_name})")
            ...
            with grpc.insecure_channel(node_agent_address) as channel:
                ...
                if response.success:
                    logger.info(f"✅ Node agent responded successfully: {response.message}")
                ...
                try:
                    ...
                    for address in node.status.addresses:
                        if address.type == 'InternalIP':
                            logger.info(f"Found node {node_name} at IP: {address.address}")
                            return address.address
                    logger.warning(f"No InternalIP found for node: {node_name}")
                except client.exceptions.ApiException as e:
                    logger.warning(f"Could not find node {node_name} in Kubernetes API: {e}")
