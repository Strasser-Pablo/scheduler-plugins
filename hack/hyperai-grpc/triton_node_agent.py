...
class TritonNodeAgent:
    ...
    def _get_thread_local_triton_client(self):
        ...
        if not hasattr(thread_local, "triton_client"):
            try:
                thread_local.triton_client = httpclient.InferenceServerClient(
                    url=self.triton_url
                )
    ...
    def _extract_node_capacity(self, node_data):
        ...
        try:
            ...
            features["node_cpu_cores"] = self._parse_cpu_to_cores(
                capacity.get("cpu", "0")
            )
            features["node_memory_gb"] = self._parse_memory_to_gb(
            ...
            features["node_allocatable_cpu_cores"] = self._parse_cpu_to_cores(
                allocatable.get("cpu", "0")
            )
            features["node_allocatable_memory_gb"] = self._parse_memory_to_gb(
    ...
    def _extract_node_metadata(self, node_data):
        ...
        try:
            ...
            features["node_instance_type"] = labels.get(
    ...
    def _get_enhanced_triton_score(self, features):
        ...
        try:
            ...
            logger.info(
                f"🧠 Using simplified features for Triton: {feature_vector_simple} (Full feature set: {len(feature_vector_full)} features available)"
            )
            ...
            result = triton_client.infer(
                model_name="scheduler_model", inputs=inputs, outputs=outputs
            )
            ...
            logger.info(
                f"🎯 Enhanced Triton inference: features={len(feature_vector)}, score={score}"
            )
            return score
    ...
    def _fallback_score(self, features):
        ...
        try:
            ...
            if features.get("node_ready", False):
                bonus += 10
            if (
                features.get("gpu_count", 0) > 0
                and features.get("node_gpu_count", 0) > 0
            ):
            ...
            logger.info(
                f"🧮 Enhanced fallback score: cpu_util={cpu_util:.2f}, mem_util={mem_util:.2f}, bonus={bonus}, score={final_score}"
            )
